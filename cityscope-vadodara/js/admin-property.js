// ===== Cityscope Vadodara — sell wizard =====
...
const state = {
  ...
  photos:[],
  price:"", negotiable:true, contact:"", amenities:[], description:"", broker:"",
};
...
// Load brokers
const fBroker = document.getElementById("fBroker");
const brokers = DB.allBrokers();
fBroker.innerHTML = `<option value="">No Broker (Owner)</option>` + brokers.map(b=>`<option value="${b.id}">${b.name}</option>`).join("");
...
document.getElementById("cta4").onclick = ()=>{
  ...
  state.broker = document.getElementById("fBroker").value;
...
document.getElementById("publishBtn").innerHTML = ICON.bolt + " Publish Property";
document.getElementById("fineprint").textContent = "🔒 Your information is safe with us";
...
document.getElementById("publishBtn").onclick = ()=>{
  ...
  const newProp = {
    ...
    rating: 5.0,
    reviews: 0,
    agent: state.broker ? DB.getBroker(state.broker).name : "You (Owner)",
    phone: state.contact ? "+91"+state.contact.replace(/\D/g,"") : "+919876543210",
    ...
    brokerId: state.broker || undefined,
