...
const DB = {
  ...
  savedIds(){ return JSON.parse(localStorage.getItem(this.KEY_SAVED) || "[]"); },
  isSaved(id){ return this.savedIds().includes(id); },
  toggleSave(id){
    let ids = this.savedIds();
    if(ids.includes(id)) ids = ids.filter(x=>x!==id);
    else ids.push(id);
    localStorage.setItem(this.KEY_SAVED, JSON.stringify(ids));
  ...
  // ---- Admin session ----
  isAdmin(){ return sessionStorage.getItem(this.KEY_ADMIN) === "1"; },
  loginAdmin(password){
    if(password === "cityscope@admin"){
      sessionStorage.setItem(this.KEY_ADMIN, "1");
      return true;
    }
    return false;
  },
  logoutAdmin(){ sessionStorage.removeItem(this.KEY_ADMIN); }
};
DB.init();

function requireAdmin(){
  if(!DB.isAdmin()){ window.location.href = "admin-login.html"; }
}
